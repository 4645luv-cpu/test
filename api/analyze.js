...
ルール:
- メロディーの特定: 1段譜ならその段。ピアノ譜(大譜表)なら上段(ト音記号)だけを追う。
- 【最重要】ヘ音記号(下段・左手・低音部)の段の音符は1つも含めてはならない。大譜表では各システムの下側の譜表を完全に無視する。
- 【最重要】和音(縦に重なった複数の符頭)は、一番上の符頭を1つだけ選び、その座標と音名を返す。1つの和音につきラベルは1つ。下側の符頭は含めない。
- すべての段(システム)を上から順に、各段内は左から右へ読み取る。
- x, y は符頭の中心のピクセル座標。画像の左上が原点 (0,0)。座標は正確に。
- pitch は「書かれている音」の科学的音名(例: "C4", "F#5", "Bb3")。中央ドは C4。
- 調号の読み取りは特に慎重に行うこと: 譜表冒頭の記号が ♯(シャープ)か ♭(フラット)かを形で確認し、その個数を正確に数える。♯なら keyFifths は正、♭なら負。
